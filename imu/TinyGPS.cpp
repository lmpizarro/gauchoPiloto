...
void TinyGPS::destination (float lat1, float long1, int distance, float course, float &lat2, float &long2){
	lat2 = 1000;
	long2 = 1000;
	float delta = distance / 6372795; 

	lat2 =asin(sin(lat1)*cos(delta)+cos(lat1)*sin(delta)*cos(course));
	long2=long1+atan2(sin(course)*sin(delta)*cos(lat1),cos(delta)-sin(lat1)*sin(lat2));
}
